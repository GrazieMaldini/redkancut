/*
 * drivers/cpufreq/cpufreq_darkonah.c
 *
...
#include <trace/events/cpufreq_darkonah.h>

struct cpufreq_darkonah_policyinfo {
	...
	struct cpufreq_darkonah_tunables *cached_tunables;
	struct sched_load *sl;
};

/* Protected by per-policy load_lock */
struct cpufreq_darkonah_cpuinfo {
	u64 time_in_idle;
...
static DEFINE_PER_CPU(struct cpufreq_darkonah_policyinfo *, polinfo);
static DEFINE_PER_CPU(struct cpufreq_darkonah_cpuinfo, cpuinfo);
...
struct cpufreq_darkonah_tunables {
	int usage_count;
...
static struct cpufreq_darkonah_tunables *common_tunables;
static struct cpufreq_darkonah_tunables *cached_common_tunables;
...
static u64 round_to_nw_start(u64 jif,
			     struct cpufreq_darkonah_tunables *tunables)
{
...
static void cpufreq_darkonah_timer_resched(unsigned long cpu,
					      bool slack_only)
{
	struct cpufreq_darkonah_policyinfo *ppol = per_cpu(polinfo, cpu);
	struct cpufreq_darkonah_cpuinfo *pcpu;
	struct cpufreq_darkonah_tunables *tunables =
 ...
 * this function.
 */
static void cpufreq_darkonah_timer_start(
	struct cpufreq_darkonah_tunables *tunables, int cpu)
{
	struct cpufreq_darkonah_policyinfo *ppol = per_cpu(polinfo, cpu);
	struct cpufreq_darkonah_cpuinfo *pcpu;
...
static unsigned int freq_to_targetload(
	struct cpufreq_darkonah_tunables *tunables, unsigned int freq)
{
	int i;
	unsigned int ret;
	unsigned long flags;
 ...
 */
static unsigned int choose_freq(struct cpufreq_darkonah_policyinfo *pcpu,
		unsigned int loadadjfreq)
{
...
{
	struct cpufreq_darkonah_policyinfo *ppol = per_cpu(polinfo, cpu);
	struct cpufreq_darkonah_cpuinfo *pcpu = &per_cpu(cpuinfo, cpu);
	struct cpufreq_darkonah_tunables *tunables =
		ppol->policy->governor_data;
	u64 now;
	u64 now_idle;
...
static unsigned int sl_busy_to_laf(struct cpufreq_darkonah_policyinfo *ppol,
				   unsigned long busy)
{
	int prev_load;
	struct cpufreq_darkonah_tunables *tunables =
...
static void cpufreq_darkonah_timer(unsigned long data)
{
	s64 now;
	...
	int cpu_load;
	int pol_load = 0;
	struct cpufreq_darkonah_policyinfo *ppol = per_cpu(polinfo, data);
	struct cpufreq_darkonah_tunables *tunables =
		ppol->policy->governor_data;
	...
	for_each_cpu(cpu, ppol->policy->cpus) {
		...
		trace_cpufreq_darkonah_cpuload(cpu, cpu_load, new_load_pct,
						  prev_l, pred_l);
	...
	if (ppol->target_freq == new_freq &&
		...
		trace_cpufreq_darkonah_already(
			max_cpu, pol_load, ppol->target_freq,
			ppol->policy->cur, new_freq);
		spin_unlock_irqrestore(&ppol->target_freq_lock, flags);
		goto rearm;
	...
	trace_cpufreq_darkonah_target(max_cpu, pol_load, ppol->target_freq,
					 ppol->policy->cur, new_freq);
...
rearm:
	if (!timer_pending(&ppol->policy_timer))
		cpufreq_darkonah_timer_resched(data, false);
...
static int cpufreq_darkonah_speedchange_task(void *data)
{
	unsigned int cpu;
	cpumask_t tmp_mask;
	unsigned long flags;
	struct cpufreq_darkonah_policyinfo *ppol;
	struct cpufreq_darkonah_tunables *tunables;
	...
	while (1) {
		...
		for_each_cpu(cpu, &tmp_mask) {
			...
			trace_cpufreq_darkonah_setspeed(cpu,
						     ppol->target_freq,
						     ppol->policy->cur);
			up_read(&ppol->enable_sem);
		}
...
static void cpufreq_darkonah_boost(struct cpufreq_darkonah_tunables *tunables)
{
	int i;
	int anyboost = 0;
...
static enum hrtimer_restart cpufreq_darkonah_hrtimer(struct hrtimer *timer)
{
	struct cpufreq_darkonah_policyinfo *ppol = container_of(timer,
			struct cpufreq_darkonah_policyinfo, notif_timer);
	int cpu;
	...
	cpu = ppol->notif_cpu;
	trace_cpufreq_darkonah_load_change(cpu);
	del_timer(&ppol->policy_timer);
	del_timer(&ppol->policy_slack_timer);
	cpufreq_darkonah_timer(cpu);
...
static int cpufreq_darkonah_notifier(
...
static struct notifier_block cpufreq_notifier_block = {
	.notifier_call = cpufreq_darkonah_notifier,
};
...
static ssize_t show_above_hispeed_delay(
	struct cpufreq_darkonah_tunables *tunables, char *buf)
{
	int i;
	ssize_t ret = 0;
	unsigned long flags;
...
static ssize_t show_hispeed_freq(struct cpufreq_darkonah_tunables *tunables,
		char *buf)
{
	return sprintf(buf, "%u\n", tunables->hispeed_freq);
}

static ssize_t store_hispeed_freq(struct cpufreq_darkonah_tunables *tunables,
		const char *buf, size_t count)
{
	int ret;
...
static ssize_t show_go_hispeed_load(struct cpufreq_darkonah_tunables
		*tunables, char *buf)
{
	return sprintf(buf, "%lu\n", tunables->go_hispeed_load);
}

static ssize_t store_go_hispeed_load(struct cpufreq_darkonah_tunables
...
static ssize_t show_min_sample_time(struct cpufreq_darkonah_tunables
		*tunables, char *buf)
{
	return sprintf(buf, "%lu\n", tunables->min_sample_time);
}

static ssize_t store_min_sample_time(struct cpufreq_darkonah_tunables
		*tunables, const char *buf, size_t count)
{
	int ret;
	unsigned long val;
...
static ssize_t show_timer_rate(struct cpufreq_darkonah_tunables *tunables,
		char *buf)
{
	return sprintf(buf, "%u\n", jiffies_to_usecs(tunables->timer_rate));
}

static ssize_t store_timer_rate(struct cpufreq_darkonah_tunables *tunables,
		const char *buf, size_t count)
{
	int ret;
	unsigned long val, val_round;
	struct cpufreq_darkonah_tunables *t;
	int cpu;
...
static ssize_t show_timer_slack(struct cpufreq_darkonah_tunables *tunables,
		char *buf)
{
	return sprintf(buf, "%u\n", jiffies_to_usecs(tunables->timer_slack_val));
}

static ssize_t store_timer_slack(struct cpufreq_darkonah_tunables *tunables,
		const char *buf, size_t count)
{
	int ret;
	unsigned long val;
...
static ssize_t show_boost(struct cpufreq_darkonah_tunables *tunables,
			  char *buf)
{
	return sprintf(buf, "%d\n", tunables->boost_val);
}

static ssize_t store_boost(struct cpufreq_darkonah_tunables *tunables,
			   const char *buf, size_t count)
{
	int ret;
	unsigned long val;
	...
	if (tunables->boost_val) {
		trace_cpufreq_darkonah_boost("on");
		if (!tunables->boosted)
			cpufreq_darkonah_boost(tunables);
	} else {
		tunables->boostpulse_endtime = ktime_to_us(ktime_get());
		trace_cpufreq_darkonah_unboost("off");
	}

	return count;
}

static ssize_t store_boostpulse(struct cpufreq_darkonah_tunables *tunables,
				const char *buf, size_t count)
{
	int ret;
	unsigned long val;
	...
	trace_cpufreq_darkonah_boost("pulse");
	if (!tunables->boosted)
		cpufreq_darkonah_boost(tunables);
	return count;
}

static ssize_t show_boostpulse_duration(struct cpufreq_darkonah_tunables
		*tunables, char *buf)
{
	return sprintf(buf, "%d\n", tunables->boostpulse_duration_val);
}

static ssize_t store_boostpulse_duration(struct cpufreq_darkonah_tunables
		*tunables, const char *buf, size_t count)
{
	int ret;
	unsigned long val;
...
static ssize_t show_io_is_busy(struct cpufreq_darkonah_tunables *tunables,
		char *buf)
{
	return sprintf(buf, "%u\n", tunables->io_is_busy);
}

static ssize_t store_io_is_busy(struct cpufreq_darkonah_tunables *tunables,
		const char *buf, size_t count)
{
	int ret;
	unsigned long val;
...
static int cpufreq_darkonah_enable_sched_input(
			struct cpufreq_darkonah_tunables *tunables)
{
	int rc = 0, j;
	struct cpufreq_darkonah_tunables *t;
...
static int cpufreq_darkonah_disable_sched_input(
			struct cpufreq_darkonah_tunables *tunables)
{
	mutex_lock(&sched_lock);
			...
			const char *buf, size_t count)
{
	...
	if (val)
		ret = cpufreq_darkonah_enable_sched_input(tunables);
	else
		ret = cpufreq_darkonah_disable_sched_input(tunables);
...
static ssize_t show_powersave_bias(struct cpufreq_darkonah_tunables *tunables,
		char *buf)
{
	return sprintf(buf, "%u\n", tunables->powersave_bias);
}

static ssize_t store_powersave_bias(struct cpufreq_darkonah_tunables *tunables,
		const char *buf, size_t count)
{
	int ret;
	unsigned long val;
...
static struct attribute *darkonah_attributes_gov_sys[] = {
...
static struct attribute_group darkonah_attr_group_gov_sys = {
	.attrs = darkonah_attributes_gov_sys,
	.name = "darkonah",
};

/* Per policy governor instance */
static struct attribute *darkonah_attributes_gov_pol[] = {
...
static struct attribute_group darkonah_attr_group_gov_pol = {
	.attrs = darkonah_attributes_gov_pol,
	.name = "darkonah",
};
...
static struct attribute_group *get_sysfs_attr(void)
{
	if (have_governor_per_policy())
		return &darkonah_attr_group_gov_pol;
	else
		return &darkonah_attr_group_gov_sys;
}

static void cpufreq_darkonah_nop_timer(unsigned long data)
{
}

static struct cpufreq_darkonah_tunables *alloc_tunable(
					struct cpufreq_policy *policy)
{
	struct cpufreq_darkonah_tunables *tunables;
...
static struct cpufreq_darkonah_policyinfo *get_policyinfo(
					struct cpufreq_policy *policy)
{
	struct cpufreq_darkonah_policyinfo *ppol =
	...
	ppol->policy_timer.function = cpufreq_darkonah_timer;
	init_timer(&ppol->policy_slack_timer);
	ppol->policy_slack_timer.function = cpufreq_darkonah_nop_timer;
	hrtimer_init(&ppol->notif_timer, CLOCK_MONOTONIC, HRTIMER_MODE_REL);
	ppol->notif_timer.function = cpufreq_darkonah_hrtimer;
...
static struct cpufreq_darkonah_tunables *get_tunables(
				struct cpufreq_darkonah_policyinfo *ppol)
{
	if (have_governor_per_policy())
...
static int cpufreq_governor_darkonah(struct cpufreq_policy *policy,
		unsigned int event)
{
	int rc;
	struct cpufreq_darkonah_policyinfo *ppol;
	...
	case CPUFREQ_GOV_POLICY_INIT:
		...
		if (tunables->use_sched_load)
			cpufreq_darkonah_enable_sched_input(tunables);
	...
	case CPUFREQ_GOV_POLICY_EXIT:
		...
		if (tunables->use_sched_load)
			cpufreq_darkonah_disable_sched_input(tunables);
	...
	case CPUFREQ_GOV_START:
		...
		ppol->last_evaluated_jiffy = get_jiffies_64();
		cpufreq_darkonah_timer_start(tunables, policy->cpu);
	...
	case CPUFREQ_GOV_LIMITS:
		...
		if (ppol->governor_enabled) {
			if (policy->min < ppol->min_freq)
				cpufreq_darkonah_timer_resched(policy->cpu,
								  true);
			ppol->min_freq = policy->min;
		}
		up_read(&ppol->enable_sem);
...
#ifndef CONFIG_CPU_FREQ_DEFAULT_GOV_DARKONAH
static
#endif
struct cpufreq_governor cpufreq_gov_darkonah = {
	.name = "darkonah",
	.governor = cpufreq_governor_darkonah,
	.max_transition_latency = 10000000,
	.owner = THIS_MODULE,
};

static int __init cpufreq_darkonah_init(void)
{
	...
	speedchange_task =
		kthread_create(cpufreq_darkonah_speedchange_task, NULL,
			       "cfdarkonah");
	...
	ret = cpufreq_register_governor(&cpufreq_gov_darkonah);
	if (ret) {
...
#ifdef CONFIG_CPU_FREQ_DEFAULT_GOV_DARKONAH
fs_initcall(cpufreq_darkonah_init);
#else
module_init(cpufreq_darkonah_init);
#endif

static void __exit cpufreq_darkonah_exit(void)
{
	int cpu;

	cpufreq_unregister_governor(&cpufreq_gov_darkonah);
	kthread_stop(speedchange_task);
	put_task_struct(speedchange_task);
...
module_exit(cpufreq_darkonah_exit);

MODULE_AUTHOR("Mike Chan <mike@android.com>");
MODULE_DESCRIPTION("'cpufreq_darkonah' - A cpufreq governor for "
